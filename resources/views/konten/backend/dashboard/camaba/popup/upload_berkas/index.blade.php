...
@include('layouts.komponen.global.script_upload')
    ...
    @if(file_exists(public_path('upload/berkas/ijazah/'.md5(Auth::user()->email).'.jpg')))
        <img src="/upload/berkas/ijazah/{!! md5(Auth::user()->email) !!}.jpg?{!! date('YmdHis') !!}"   id='gambar' class='img-rounded'   style='width:140px;height:160px;border:3px solid #aaa;' />
    @else
...
<div class="fileUpload btn btn-primary">
    ...
    <input data-form-data='{"_token": "{!! csrf_token() !!}", "ref_jenis_berkas_id" : "1" }' 
        id="fileupload" 
        type="file"
        name="files" 
...
<div style='display:none;' id='tombol_upload'>
    ...
    @else
        <div class='alert alert-danger'>error! masalah pada permision di folder <b>public/upload/berkas/ijazah/</b> </div>
    @endif
</div>
...
//function start here
   ...
   var skrg_int = skrg.getDate()+""+skrg.getHours()+""+skrg.getMinutes()+""+skrg.getSeconds();
    ...
    $('#fileupload').val('');
       ...
       $('#gambar').attr('src', '/upload/berkas/ijazah/{!! md5(Auth::user()->email).".jpg?" !!}'+skrg_int);
