@include($konten_view.'komponen.nav_atas')
...
@include('layouts.komponen.global.script_upload')
    ...
    @if(file_exists(public_path('upload/berkas/surat_keterangan_lulus/'.md5(Auth::user()->email).'.jpg')))
        <img src="/upload/berkas/surat_keterangan_lulus/{!! md5(Auth::user()->email) !!}.jpg?{!! date('YmdHis') !!}"   id='gambar' class='img-rounded'   style='width:140px;height:160px;border:3px solid #aaa;' />
    @else
...
<div class="fileUpload btn btn-primary">
    <span> <i class="fa fa-link"></i> pilih & upload file </span>
    <input data-form-data='{"_token": "{!! csrf_token() !!}", "ref_jenis_berkas_id" : "2" }' 
        id="fileupload" 
        type="file"
        name="files" 
        data-url="{!! URL::route('camaba.do_upload_berkas') !!}" 
...
<div style='display:none;' id='tombol_upload'>
    @if(File::isWritable(public_path('upload/foto')))
        <button class="btn btn-primary" id="do_upload">
                            <i class="fa fa-floppy-o"></i>  SIMPAN</button>
    @else
        <div class='alert alert-danger'>error! masalah pada permision di folder <b>public/upload/berkas/surat_keterangan_lulus/</b> </div>
    @endif
</div>
...
//function start here
   ...
   var skrg_int = skrg.getDate()+""+skrg.getHours()+""+skrg.getMinutes()+""+skrg.getSeconds();
    ...
    $('#fileupload').val('');
       ...
       $('#gambar').attr('src', '/upload/berkas/surat_keterangan_lulus/{!! md5(Auth::user()->email).".jpg?" !!}'+skrg_int);
